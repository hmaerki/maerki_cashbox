...
namespace Raptorious.SharpMt940Lib
{
  ...
  public class CustomerStatementMessage
  {
    ...
    public CustomerStatementMessage(CustomerStatementMessage customerStatementMessage)
    {
      ...
      this.Account = customerStatementMessage.Account;
      this.ClosingAvailableBalance = customerStatementMessage.ClosingAvailableBalance;
      this.ClosingBalance = customerStatementMessage.ClosingBalance;
      this.Description = customerStatementMessage.Description;
      this.ForwardAvailableBalance = customerStatementMessage.ForwardAvailableBalance;
      this.OpeningBalance = customerStatementMessage.OpeningBalance;
      this.RelatedMessage = customerStatementMessage.RelatedMessage;
      this.SequenceNumber = customerStatementMessage.SequenceNumber;
      this.StatementNumber = customerStatementMessage.StatementNumber;
      this.TransactionReference = customerStatementMessage.TransactionReference;
      this.Transactions = customerStatementMessage.Transactions;
    }
    ...
    public void SetSequenceNumber(string transactionData, IFormatProvider cultureInfo)
    {
      ...
      if (ValueConverter.TryParseInteger(transaction[0], cultureInfo, out statementNumber))
      {
      ...
      if (transaction.Length > 1)
      {
        ...
        if (ValueConverter.TryParseInteger(transaction[1], cultureInfo, out sequenceNumber))
        {
