...
namespace Raptorious.SharpMt940Lib
{
  ...
  public static class Mt940Parser
  {
    ...
    public static ICollection<CustomerStatementMessage> Parse(IMt940Format format, string file, CultureInfo cultureInfo, Parameters mt940Parameters = null)
    {
      ...
      Encoding encoding = mt940Parameters != null ? mt940Parameters.Encoding : Encoding.UTF8;
      using (StreamReader reader = new StreamReader(File.OpenRead(file), encoding))
      {
        ...
        Parameters mt940Params)
    {
      ...
      foreach (String[] line in data)
      {
        ...
        Action addAndNullTransactionIfPresent = () =>
        {
            ...
            switch (tag)
            {
              ...
              case ":NS:": // Raiffeisen
                ...
                */
                ...
                {
                  string s1 = s;
                  if (mt940Params.LeadingLineNumber)
                  {
                    Trace.Assert(s.Length >= 2);
                    Trace.Assert(char.IsNumber(s[0]));
                    Trace.Assert(char.IsNumber(s[1]));
                    s1 = s.Substring(2);
                  }
                  s1 = s1.Trim();
                  if (s1.Length == 0)
                ...
                string text = string.Join(" / ", lines2);
                if (transaction == null)
                {
                  customerStatementMessage = customerStatementMessage.SetDescription(text);
                }
                else
                {
      ...
      if (mt940Params.Codes.Count > 0)
      {
        ...
        foreach (var item in customerStatementList)
        {
          ...
          {
            Mt940Parser.Parse86Details(tr, mt940Params, constantCode);
          }
        }
      }
