...
namespace Raptorious.SharpMt940Lib
{
  ...
  public class Money : IEquatable<Money>, IComparable, IComparable<Money>
  {
    ...
    public Money(string value, Currency currency)
    {
      ...
      value = value.Replace(",", ".");
      var formattedValue = string.Format(CultureInfo.InvariantCulture, "{0:C}", value);
      this.Value = ValueConverter.ParseDecimal(formattedValue, CultureInfo.InvariantCulture);
    ...
    {
      return string.Format("{0} {1}", this.Currency.Code, this.Value);
    }
  }
}